...
public class ChessGame4Players : BaseChessGame
{
    ...
    protected override async Task<bool> Moving(string personId, int oldRow, int oldCol, int newRow, int newCol)
    {
        ...
        if (piece.Type == PieceType.Pawn && person.Color == "#000000" && newRow == 15)
        {
        ...
        else if (piece.Type == PieceType.Pawn && person.Color == "#DD4CEE" && newCol == 15)
        {
            var temporary = new Queen(piece.OwnerId);
            piece = temporary;
        }
        else if (piece.Type == PieceType.Pawn && person.Color == "#7074D5" && newCol == 0)
        {
            var temporary = new Queen(piece.OwnerId);
            piece = temporary;
        }
